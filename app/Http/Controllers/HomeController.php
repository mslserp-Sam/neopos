...
class HomeController extends Controller
{
    ...
    public function neo_tag_upline_history(DataTables $datatable, Request $request){
        ...
        $neo = $query->where('user_type', 'Neopreneur')->where('upline', $getUser->referal_code);
        $query = $query->where('user_type', 'provider')->where('upline', $neo->referal_code);
        $query = null;
        return $datatable->eloquent($query)
        ->editColumn('display_name', function($query){
            return '<a class="btn-link btn-link-hover" >'.$query->display_name.'</xa>';
        })
        ->filterColumn('display_name',function($query,$keyword){
            $query->where('display_name','like','%'.$keyword.'%');
        })
